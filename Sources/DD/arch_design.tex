...
As already discussed in the RASD and showed in this document, we decided that the overall logic of the service will be implemented in Java using the JEE technology. This has the main advantage of having a single programming language and set of libraries on the server side that are able to deal with View, Controller and also the Model providing some techniques that are capable of abstracting the database using the Object Oriented Programming Paradigm. \newline
The View will be implemented using the Java Server Faces technology exposing the methods in the GUI Logic presented in the previous sections. The Controller will work tightly coupled with the rest of the components again using Java and JEE, thus enabling a simpler and a coherent way to establish a communication between each tier. \newline Moreover, the developers can use an IDE to implement the actual code and use enhanced feature of the IDE itself such as automated code inspection and testing (using frameworks such as JUnit), thus allowing a more stable and bug-free code since the very first release. 
