...
Alice is a taxi driver and she has just been hired by the company. She has already provided her information about her driving license ID and her taxi car ID. Bill, the system administrator logs into the system and insert the data supplied by Alice into the DB. Upon the click of the ``Confirm'' button, the system connects to the external database and checks if the data is present and valid. As expected all the information are good and the system writes the data into its DB and displays a confirmation message to Bill, now Alice is able to login into the system and start to do her job.
\pagebreak
...
\begin{minipage}[b]{11cm}
\begin{enumerate}
\item The system administrator open the administration page
\item The system administrator insert his/her credentials.
\end{enumerate}
\end{minipage}
}
{The system administrator is now capable of manage the taxi driver and their car information.}
{
\begin{minipage}[b]{11cm}
\begin{enumerate}
\item Invalid credentials
...
\item The system administrator press button ``Insert new Driver''
\item The system administrator insert the taxi driver license ID and car ID.
\item The system check the validity of the data, it can happen:
\item The system connect to the external database and checks the consistency of the data
\item The system stores the data in the DB.
\end{enumerate}
\end{minipage}
}
{The taxi driver is now able to log into the system and thus he/she can start to use it.}
{
\begin{minipage}[b]{11cm}
\begin{enumerate}
