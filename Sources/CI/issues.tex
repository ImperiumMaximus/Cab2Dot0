...
This method begins at \codeline{394} and ends at \codeline{426}, below there is a list of violations found between this line range.
\begin{enumerate}
	\item \ref{C13} \codeline{399}. Line length exceeds 80 characters and can be broken at ``\texttt{;}'' just before the condition statement. \sourcesnippet{399-399}{\ref{C13} violation at \codeline{399}}{Code/DeploymentDescriptorNode.java}	
	\item \ref{C31} \codeline{401}, \codeline{403}, \codeline{411}, \codeline{416}, \codeline{419} and \codeline{423}. These statements don't check properly if either \texttt{element} or \texttt{element.getQName()} is not \texttt{null} before using only one of them or both.
	...
	\item \ref{C33} \codeline{411}. Move this statement to the beginning of the block.
	\sourcesnippet{411-411}{\ref{C33} violation at \codeline{411}}{Code/DeploymentDescriptorNode.java}	
	\item \ref{C56} \codelines{399}{400}. Loop isn't well formed since increment statement is inside its block of code rather than in initialization. \sourcesnippet{399-400}{\ref{C56} violation at \codelines{399}{400}}{Code/DeploymentDescriptorNode.java}
\end{enumerate}

\subsection{Issues related to method \texttt{setElementValue()}}
This method begins at \codeline{442} and ends at \codeline{491}. Here it's a list of violations found within this method:
\begin{enumerate}
	...
	\item \ref{C9} \codeline{443} and \codeline{468}. These lines are indented using tabs, consider replacing them with spaces.
	...
	\item \ref{C13} \codelines{470}{471}. Consider wrapping this comment in order not to exceed 80 characters in a single line.
	...
	\item \ref{C14} \codeline{443}. Commented statement length exceeds 120 characters, it is advisable to split it before 3\textsuperscript{rd} ``\texttt{+}''.
	\sourcesnippet{443-443}{\ref{C14} violation at \codeline{443}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C14} \codeline{451}. String concatenation statement length inside this function call exceeds 120 characters, consider wrapping it at 1\textsuperscript{st} ``\texttt{+}''.
	\sourcesnippet{451-451}{\ref{C14} violation at \codeline{451}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C19} \codeline{443}. Consider either to remove this comment from statement or add a reason why it is there and optionally add a date when it can be removed.
	\sourcesnippet{443-443}{\ref{C19} violation at \codeline{443}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C31} \codeline{447}, \codeline{448}, \codeline{458}, \codelines{460}{461}, \codelines{466}{467}, \codelines{472}{473} and \codelines{487}{488}. These statements don't check properly if either \texttt{element} or \texttt{element.getQName()} is not \texttt{null} before using only one of them or both.
	...
	\item \ref{C31} \codeline{458}, \codelines{460}{461}, \codelines{472}{473}, \codeline{486} and \codelines{487}{488}. These statements don't check properly if \texttt{value} is not \texttt{null} before using it.
	\sourcesnippet{458-458}{\ref{C31} violation at \codeline{458}}{Code/DeploymentDescriptorNode.java}
	\sourcesnippet{460-461}{\ref{C31} violation at \codelines{460}{461}}{Code/DeploymentDescriptorNode.java}
	\sourcesnippet{472-473}{\ref{C31} violation at \codelines{472}{473}}{Code/DeploymentDescriptorNode.java}
	\sourcesnippet{486-486}{\ref{C31} violation at \codeline{486}}{Code/DeploymentDescriptorNode.java}
	\sourcesnippet{487-488}{\ref{C31} violation at \codelines{487}{488}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C31} \codeline{458}, \codelines{460}{461}, \codelines{472}{473}, \codeline{486} and \codelines{487}{488}. Following statements don't properly check if returned value by \texttt{getDescriptor()} function is not \texttt{null} before using it.
	\sourcesnippet{467-468}{\ref{C31} violation at \codelines{467}{468}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C33} \codeline{468}. Move this statement to the beginning of \texttt{catch} block.
	\sourcesnippet{465-468}{\ref{C33} violation at \codeline{468}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C42} \codeline{477} and \codeline{482}. Generic \texttt{``Error occurred''} message, consider to change it with a more explicative one.
	...
	\item \ref{C44} \codeline{486}. Consider to use \texttt{String.isEmpty()} method which is already provided by Java standard library.
	\sourcesnippet{486-486}{\ref{C44} violation at \codeline{486}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C50} \codeline{479}. Catching a \texttt{Throwable} can lead to errors, consider to use at least \texttt{Exception} or more specific subclasses, this can also improve code readability.  
...
This method begins at \codeline{512} and ends at \codeline{537} and a list of violations regarding this method is the following:
\begin{enumerate}
	...
	\item \ref{C13} \codeline{521}, \codeline{526} and \codeline{533}. Consider to split this function calls at ``\texttt{,}''. 
	...
	\item \ref{C14} \codeline{517}. String concatenation statement length inside function call exceeds 120 characters, consider to wrap it at 2\textsuperscript{nd} ``\texttt{+}''.
	\sourcesnippet{517-517}{\ref{C14} violation at \codeline{517}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C31} \codeline{522}, \codeline{527} and \codeline{534}. These statements don't check properly if \texttt{value} is not \texttt{null} before using it.
	...
	\item \ref{C31} \codelines{521}{522}, \codelines{526}{527} and \codelines{533}{534}. These statements don't check properly if \texttt{target} is not \texttt{null} before using it.
	...
	\item \ref{C31} \codeline{521}, \codeline{526} and \codeline{533}. These statements don't check properly if \texttt{methodName} is not \texttt{null} before using it.
	...
	\item \ref{C42} \codeline{517}. This message is misleading. It says that it's already in its target method of class but its invocation hasn't occurred yet. This could potentially lead to confusion if there are problems invoking this method at runtime using reflection, i.e. if at least one of exceptions is caught.
	\sourcesnippet{517-517}{\ref{C42} violation at \codeline{517}}{Code/DeploymentDescriptorNode.java}
	\item \ref{C53} \codelines{524}{535}. Consider to catch all checked exceptions within the method rather than propagating the last one to caller in case all tries fail. If an error is unrecoverable, a good solution is to throw a dedicated exception with a meaningful message containing an explanation about the exceptional event. 
...
\item \ref{C18} \codeline{805} and \codeline{815}. These comments aren't useful to explain their blocks of code.
