...
\begin{enumerate}
	...
	\item This class has too many methods, consider refactoring it.
	\item \codeline{94} and \codeline{100}. Avoid using implementation types like \texttt{``Hashtable''}; instead use an interface.
...
\begin{enumerate}
	\item \codeline{394}. Add \texttt{``@Override''} annotation above this method signature.
	\sourcesnippet{394-394}{Missing \texttt{``@Override''} annotation in \texttt{handlesElement()}}{Code/DeploymentDescriptorNode.java}
	\item \codeline{401}. Consider using a local variable as returned value by \break\texttt{element.getQName()} method, instead of chaining function calls.
	\sourcesnippet{401-401}{\texttt{element.getQName().equals(\dots)} methods chaining}{Code/DeploymentDescriptorNode.java}
	\item \codeline{399}. Consider declaring a variable using an \texttt{Enumeration<String>} type in order to avoid casts in \codeline{400}. Thus to improve readability.
...
\begin{enumerate}
	\item \codeline{442}. Add \texttt{``@Override''} annotation above this method signature.
	...
	\item \codeline{450} and \codeline{457}. Refactor this code not to nest more than 3 \texttt{if}/\texttt{for}/\texttt{while}/\texttt{switch}/\texttt{try} statements.
	\item \codeline{450}, \codeline{451}, \codeline{460}, \codeline{466}, \codeline{472}, \codeline{475}, \codeline{477}, \codeline{480}, \codeline{482} and \codeline{487}. Consider using a local variable as returned value by \texttt{DOLUtils.getDefaultLogger()} method, instead of chaining function calls.
	...
	\item \codeline{490}. Avoid unnecessary \texttt{return} statements.
	\sourcesnippet{490-490}{Unnecessary \texttt{return} statement at end of method}{Code/DeploymentDescriptorNode.java}
	\item \codeline{444}. Consider declaring a variable using \texttt{Map<String, String>} type in order to avoid casts in \codeline{458}. Thus to improve readability.
...
\begin{enumerate}
	\item \codeline{516} and \codeline{517}. Consider using a local variable as returned value by \break \texttt{DOLUtils.getDefaultLogger()} method, instead of chaining function calls.
...
\begin{enumerate}
\item \codeline{643}. Avoid to use a chain of methods
\sourcesnippet{643-643}{Concatenation of methods}{Code/DeploymentDescriptorNode.java}
\item \codeline{657}. Consider using a local variable as returned value by \texttt{DOLUtils.getDefaultLogger()} method, instead of chaining function calls.
\sourcesnippet{657-657}{\texttt{DOLUtils.getDefaultLogger().log(\dots)} methods chaining}{Code/DeploymentDescriptorNode.java} 
\end{enumerate}
\subsection{Other problems related to method \texttt{writeEjbReferenceDescriptors()}}
\begin{enumerate}
\item \codeline{806}. Use \texttt{<EjbReference>} type when a \texttt{Set} and a \texttt{HashSet} are declared to avoid errors caused by insertion of different type elements.
\sourcesnippet{806-806}{Add diamond operator \texttt{<EjbReference>}}{Code/DeploymentDescriptorNode.java}
\end{enumerate}